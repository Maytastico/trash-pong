...
Websockets wären hier ein Protokoll, welche einen verbindungsorientierten Ansatz verfolgen.
...
Ein Spieler kann dabei mehrere Räume erstellen, so ist es diesen auch möglich, in mehreren Räumen gleichzeitig zu sein.
...
welches die Funktionen von Websockets mit Räumen erweitert.
Das unter dem Socket.io liegende Engine.IO ermöglicht dabei latenzarme Kommunikation
...
Zur Authentifizierung werden JWT-Tokens verwendet. 
Der Vorteil dabei ist, dass der Server keine Session speichern muss.
Der übertragene Token kann vom Client gespeichert werden. 
Die Informationen im Token sind signiert und würden bei einer Manipulation des Tokens ungültig werden \cite[]{jwt-rfc}. 
Ein Token wird für den Nutzer übertragen, und sobald der Start Game-Button gedrückt wird, 
wird auch ein Token für den Raum erstellt, den beide Nutzer empfangen. 
Mit diesem können sie dann Nachrichten innerhalb des Raumes senden.
Ein anderer Vorteil der JWT-Tokens ist, dass Daten im Token gespeichert werden können,
sodass kein weiterer Datenbankzugriff notwendig ist.
Damit kann die anfragende Person direkt identifiziert werden.
...
Außerdem können relationale Datenbank Datenkonsistenz überzeugen. 
Die Verwendung von relationalen Datenbanken ermöglicht es auch, 
verschiedene Datenbanksysteme zu integrieren, ohne die Anwendungslogik ändern zu müssen.
